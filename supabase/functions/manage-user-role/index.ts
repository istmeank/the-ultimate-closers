...
serve(async (req) => {
  ...
  try {
    ...
    // Client lié à l'utilisateur pour récupérer son identité
    ...
    if (!userId || !role || !action) {
      return new Response(
        JSON.stringify({ error: 'Paramètres requis: userId, role, action' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    if (!['admin', 'closer', 'developer'].includes(role)) {
      return new Response(
    ...
    // Empêcher modification des owners (sécurité)
    ...
    if (targetIsOwner && !isOwner) {
      return new Response(
        JSON.stringify({ error: 'Impossible de modifier un owner (réservé aux owners)' }),
        { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
