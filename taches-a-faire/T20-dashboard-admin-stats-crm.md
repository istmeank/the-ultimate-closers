# T20 — Dashboard admin stats CRM (leads, deals, CA, taux conversion)

**Priorité** : P6 admin
**Agent responsable** : `frontend-react`
**Skill** : `react-shadcn-design-system`
**Modèle Claude** : `sonnet-4-6` — Charts Recharts + KPIs
**Skills Cowork (Claude PC)** : frontend-design, ui-ux-pro-max, design:design-handoff
**Effort estimé** : 2-3h
...
**Tâche T20** : étendre le dashboard admin avec stats CRM globales.

**Mission** :
Modifie `src/components/admin/Dashboard.tsx` ou crée widget `AdminCRMStats.tsx` :
- **Leads totaux** : count global
- **Deals en cours** : count où stage IN ('qualified', 'proposal', 'negotiation')
- **CA généré** : SUM amount_cents WHERE stage = 'won' (sur période)
- **Taux conversion** : leads won / leads total
- **Distribution leads par status** : pie chart Recharts
- **Évolution leads sur 30j** : line chart Recharts
- Filtres période : 7j / 30j / 90j / All

**Délègue à frontend-react** (utilise Recharts depuis CDN, skill `react-shadcn-design-system`).

**Critères d''acceptation** :
- [ ] 4 KPIs corrects (vérif manuelle SQL)
...
### 2. Tests spécifiques à cette tâche
   - [ ] 4 KPIs vérifiés via SQL direct
   - [ ] Charts Recharts responsive
   - [ ] Filtre période (7j/30j/90j/All) fonctionnel
   - [ ] RLS admin only validé
...
- [ ] **Commit Git conventionnel** : `<type>(<scope>): <résumé>` (ex: `feat(T20): <résumé concis>`)
