...
| T05 | Triggers log_appointment + log_deal_interaction | database-postgres | **haiku** | ✅ completed (appliquée en base le 2026-08-09) | Fichier `20260809000001_tuc_v2_triggers_log_interactions.sql` créé, vérifié et appliqué en session 37 (2026-08-09). Élargissement CHECK `interactions.type` (ajout `'note'`), triggers `log_appointment_as_interaction` / `log_deal_as_interaction`. Trois ADR dérivés (ADR-040 kanban pipeline 7 stades, ADR-041 métadonnées JSONB, ADR-042 qualification/température). Deux BLOCKERS ouverts (BLOCKER-015 définition affaire active, BLOCKER-016 ordonnancement CHECK). Migration vérifiée : interactions.metadata ✅, deals.stage 7 valeurs ✅, CHECK 'note' ✅. Sécurité : 0 nouvelles alertes. |
