...
| 2 | [Messagerie Multi-canaux](./02-messagerie-multicanaux/PLAN.md) | 🔴 Non implémenté | V2-V3 | `integrations` + `ia-orchestration` | Cadrage WhatsApp Business API + ADR compliance |
| 3 | [Matching IA Prospects ↔ Closers](./03-matching-ia/PLAN.md) | 🔴 Non implémenté | V3 | `ia-orchestration` + `database-postgres` | Modélisation `CloserProfile` et `ProspectProfile` |
| 4 | [Préparation Meet & Coaching](./04-meet-coaching/PLAN.md) | 🟡 Briefing/calendrier partiels | V2 | `integrations` + `ia-orchestration` | Intégration Google Calendar (token table existe) + transcription |
